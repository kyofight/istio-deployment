...
KOPS_VERSION="1.15.0" #$(curl -s https://api.github.com/repos/kubernetes/kops/releases/latest | grep tag_name | cut -d '"' -f 4)
KOPS_URL="https://github.com/kubernetes/kops/releases/download/${KOPS_VERSION}/${KOPS_FLAVOR}"

KUBECTL_VERSION="v1.17.0" #$(curl -s https://storage.googleapis.com/kubernetes-release/release/stable.txt)
KUBECTL_URL="https://storage.googleapis.com/kubernetes-release/release/${KUBECTL_VERSION}/bin/linux/amd64/kubectl"

TERRAFORM_VERSION="0.11.0" #$(curl -s https://checkpoint-api.hashicorp.com/v1/check/terraform | jq -r -M '.current_version')
...
helm_setup(){
    ...
    log "INFO: Download HELM"
    curl ${HELM_URL} > helm.sh || log  "ERROR: Downlaod failed" $?
    log "INFO: Download Complete"

    chmod 700 helm.sh || log "ERROR: Cant set the executable permission" $?

    ./helm.sh || log "ERROR: helm installation failed" $?
}
...
verify_install(){
    ...
    log "INFO: VERIFY HELM"

    helm version || log "ERROR: helm verification failed" $?

    log "INFO: Validation Successful !!!"

    rm -f terraform_${TERRAFORM_VERSION}_linux_amd64.zip || log "ERROR: terraform zip Cleanup failed" $?

    rm -f helm.sh || log "ERROR: helm.sh Cleanup failed" $?
